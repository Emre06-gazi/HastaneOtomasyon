...
public class Tahlil extends javax.swing.JPanel {
    ...
    private void jButton1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButton1MouseClicked
        ...
        String sorgu;
        if(kelime.length() == 0){
            sorgu = "SELECT hastaneadi, klinikadi, klinikno, doktoradsoyad, randevutarihi FROM randevubilgileritum(?,?) ORDER BY randevutarihi";
        }
        else{
            sorgu = "SELECT hastaneadi, klinikadi, klinikno, doktoradsoyad, randevutarihi FROM randevubilgileritum(?,?) WHERE hastaneadi LIKE ? OR klinikadi LIKE ? OR CONCAT(klinikno,'') LIKE ? OR doktoradsoyad LIKE ? OR TO_CHAR(randevutarihi, 'DD-MM-YYYY HH24:MI') LIKE ? ORDER BY randevutarihi";
        }
        Query db = new Query();
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        if(row != -1){
            int evet = JOptionPane.showOptionDialog(this,"Taracýnýzýn açýlmasýný onaylýyor musunuz?", "", 0,JOptionPane.INFORMATION_MESSAGE,null,new String[] {"Evet","Hayýr"},null);
            if(evet == 0){
                String temp = (String) jTable1.getValueAt(row, 4);
                Query db = new Query();
                Connection con = db.getCon();
                try {
                    PreparedStatement stmt = con.prepareStatement("SELECT dosyano FROM tahlil WHERE tahlilno IN (SELECT tahlilno FROM randevu WHERE hastano = ? AND tarih = TO_DATE(?, 'DD-MM-YYYY HH24:MI'))");
                    stmt.setLong(1, kullanici);
                    stmt.setString(2, temp);
                    ResultSet result = stmt.executeQuery();
                    String link = "www.google.com";
                    while (result.next()){
                        link = result.getString("dosyano");
                    }
                    try {
